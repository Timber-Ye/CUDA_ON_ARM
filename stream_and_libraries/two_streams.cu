...
int main(void){
    ...
    CHECK(cudaMalloc((void **)&d_a_1, sizeof(int) * N));
    CHECK(cudaMalloc((void **)&d_b_1, sizeof(int) * N));
    CHECK(cudaMalloc((void **)&d_c_1, sizeof(int) * N));
    ...
    for(int i=0; i<FULL_DATA_SIZE; i+= N*2){
        ...
        CHECK(cudaMemcpyAsync(d_a_1, h_a+i+N, sizeof(int) * N, cudaMemcpyHostToDevice, stream_1));

        CHECK(cudaMemcpyAsync(d_b_0, h_b+i, sizeof(int) * N, cudaMemcpyHostToDevice, stream_0));
        CHECK(cudaMemcpyAsync(d_b_1, h_b+i+N, sizeof(int) * N, cudaMemcpyHostToDevice, stream_1));

        kernel<<<N/256,1024,0,stream_0>>>( d_a_0, d_b_0, d_c_0 );
        kernel<<<N/256,1024,0,stream_1>>>( d_a_1, d_b_1, d_c_1 );

        CHECK(cudaMemcpyAsync(h_c+i, d_c_0, N * sizeof(int), cudaMemcpyDeviceToHost, stream_0));
        CHECK(cudaMemcpyAsync(h_c+i+N, d_c_1, N * sizeof(int), cudaMemcpyDeviceToHost, stream_1));
    }
    ...
    CHECK( cudaFree( d_a_1 ) );
    CHECK( cudaFree( d_b_1 ) );
    CHECK( cudaFree( d_c_1 ) );
